...
function RegistrationForm() {
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formState.billingAddress.taxNumber) {
      console.log("if billtaxnumber ifen belül");
      console.log(formState.billingAddress.taxNumber);
    ...
    const canSubmit = Object.entries(formState).every(([fieldName, fieldValue]) => {
      let hasValue = typeof fieldValue === 'object'
      ...
      if (typeof fieldValue === "object") {
        Object.entries(fieldValue as Record<string, string>).map(([key, value], index) => {
          if (key !== "taxNumber") {
            if (value.trim() !== '') {
              hasValue = true;
            } else {
      ...
      console.log("fieldName" + fieldName + " fieldValue" + JSON.stringify(fieldValue) + " hasValue" + hasValue);
      if (!hasValue) {
        const setError = fieldErrorSetters[fieldName];
        if (setError) {
